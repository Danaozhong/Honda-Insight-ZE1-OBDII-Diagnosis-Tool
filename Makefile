...
SKETCH=H:/Projekte/Eclipse/ZE1OBD/user/ZE1OBD.ino
...
ifndef ESP_ROOT
  ...
  ifeq ($(OS), Windows_NT)
    # TODO Adapt this for your PC. Sorry. I have no clue about makefiles...
    ARDUINO_ROOT = C:/Users/Clemens/AppData/Local/Arduino15 
  else ifeq ($(OS), Darwin)
   ...
   # TODO Adapt this for your PC. Sorry. I have no clue about makefiles...
  ARDUINO_ESP_ROOT = C:/Users/Clemens/Documents/Arduino
  ESP_ROOT = $(ARDUINO_ESP_ROOT)/hardware/espressif/$(CHIP)
  ifeq ($(ESP_ROOT),)
    $(error No installed version of $(ARDUINO_ESP_ROOT) $(CHIP) Arduino found)
  endif
...
USER_INC := $(filter-out $(IGNORE_PATTERN),$(shell find $(SKETCH_DIR) $(dir $(LIBS)) -name "*.h" -o -name "*.hpp"))
USER_SRC := $(SKETCH) $(filter-out $(IGNORE_PATTERN),$(shell find $(SKETCH_DIR) $(LIBS) -name "*.S" -o -name "*.c" -o -name "*.cpp"))

# Clemens Patch append own libraries
USER_INC += G:/Programme/Libraries/Boost/v1.66.0/
#USER_SRC += $(ESP_LIBS)/libraries/BLE/src/
...
#$(info * My Includes are $(USER_INC_DIRS) *)
...
$(info * My Variant is $(INCLUDE_VARIANT) *)
...
$(info * My Include dirs are $(C_INCLUDES) *)
